...
public class WouldYouRatherInventory implements InventoryHolder {
    ...
    public static final int[] OPTION_ONE_SELECTION_AREA = {28, 29, 30, 36, 37, 38, 46, 47, 48};
    ...
    public static final int[] OPTION_TWO_POSITIVE_AREA = {32, 33, 34};
    public static final int[] OPTION_TWO_NEGATIVE_AREA = {50, 51, 52};
    ...
    public WouldYouRatherInventory(WouldYouRatherPlugin plugin, Choice[] option1, Choice[] option2, Component inventoryName) {
        ...
        for (int i : OPTION_ONE_POSITIVE_AREA) {
            ItemStack item = createItem(option1[0].runnable().getChoice().getDescription(), TextColor.color(0, 255, 0));
            this.inventory.setItem(i, item);
        }
        for (int i : OPTION_ONE_NEGATIVE_AREA) {
            ItemStack item = createItem(option1[1].runnable().getChoice().getDescription(), TextColor.color(255, 0, 0));
            this.inventory.setItem(i, item);
        }


        for (int i : OPTION_TWO_POSITIVE_AREA) {
            ItemStack item = createItem(option2[0].runnable().getChoice().getDescription(), TextColor.color(0, 255, 0));
            this.inventory.setItem(i, item);
        }
        for (int i : OPTION_TWO_NEGATIVE_AREA) {
            ItemStack item = createItem(option2[1].runnable().getChoice().getDescription(), TextColor.color(255, 0, 0));
            this.inventory.setItem(i, item);
        }
    }
